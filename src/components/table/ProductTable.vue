...
<template>
  <table class="relative w-full text-left">
    ...
    <tbody>
      <tr class="cursor-pointer border-t bg-white hover:bg-[#f5f5f5]" v-for="(item, index) in data" :key="item._id"
        ...
        <td class="px-6 py-2 truncate">{{ item.model }}</td>
        <td class="px-6 py-2 truncate">{{ item.text }}</td>
